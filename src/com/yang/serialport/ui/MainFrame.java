...
import com.yang.serialport.db.Dao;
	...
	private void openSerialPort(java.awt.event.ActionEvent evt) {
		...
		timer.schedule(new TimerTask(){
			public void run(){
				...
				data=sum/times;
				data =(float)(Math.round(data*10000))/10000;//精确到4位小数
				sum=0;//清空
				times=0;//清空
				System.out.println(data);
				dao.insertData(data,new Date());
				dataView.append(data + "\r\n");
				dataView.setCaretPosition(dataView.getDocument().getLength());//将滚动条滚动到底部
			}
		}, 1000,1000);//1秒钟
			...
			case SerialPortEvent.DATA_AVAILABLE: // 1 串口存在可用数据
				...
				try {
					if (serialport == null) {
						...
						String substring = new String(data).substring(3, 10);
